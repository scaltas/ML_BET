...
public class PredictorService : IPredictorService
{
    ...
    {
        return PredictFirstHalfWithLinearRegression(preprocessedMatches, odds, scored);
    }
    ...
    private double PredictFirstHalfWithLinearRegression(List<PreprocessedMatch> preprocessedMatches, double odds, bool scored)
    {
        double[] inputs = preprocessedMatches.Select(match => match.Odds).ToArray();
